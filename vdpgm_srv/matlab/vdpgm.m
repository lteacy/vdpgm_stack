...
%****************************************************************
%******* ADDED BY LUKE TEACY TO SURPRESS OUTPUT
if isfield(opts, 'suppress_output')
   my_disp([],opts.suppress_output);
end
%****************************************************************
...
if ~ isfield(opts, 'get_E_pi') % added by Luke Teacy - gets expected weights
  opts.get_E_pi = 0;
end
...
%     q_of_z = sort_q_of_z(data, q_of_z, opts);  
  ...
  my_disp(['SIS: ' num2str(r) ';  best f = ' num2str(min_f) ';  best Nc = ' num2str(hp_posterior.Nc)])
end
my_disp('SIS done.')
...
while 1
  ...
  my_disp(['finding the best one.... done.  component ' num2str(c) ' was split.'])
...
while 1
  ...
  for c=candidates_for_splliting(1:min(c_max, length(candidates_for_splliting)))
    my_disp(['### start splitting k=' num2str(c) '...'])
    ...
    if free_energy_improved(free_energy, new_free_energy, 0, opts)
      my_disp(['### splitting k=' num2str(c) ' improved the free energy.  ' ...
           num2str(free_energy) ' -> ' num2str(new_free_energy)])
      break
    else
      my_disp(['### splitting k=' num2str(c) ' did not improve the free energy.'])
    end
  end
  my_disp('### end splitting')
  if free_energy_improved(free_energy, new_free_energy, 0, opts)
    free_energy = new_free_energy;
    hp_posterior = new_hp_posterior;
    data = new_data;
  ...
  for i=1:min(c_max, size(candidates_for_merging,2))
    ...
    c1 = pair(1); c2 = pair(2);
    my_disp(sprintf('### start merging c1=%d, c2=%d ...', c1, c2))
    new_q_of_z = q_of_z;
    ...
    [new_free_energy, new_hp_posterior] = update_posterior2(data, ...
                                                      ...
                                                      hp_prior, opts);
    if free_energy_improved(free_energy, new_free_energy, 0, opts)
      my_disp(sprintf('### merging c1=%d, c2=%d improved the free energy.  %0.5g -> %0.5g', ...
                   c1, c2, free_energy, new_free_energy))
      break
    else
      my_disp(sprintf('### merging c1=%d, c2=%d did not improve the free energy.', c1, c2))
    end
  end
  my_disp(['### end merging'])
  if free_energy_improved(free_energy, new_free_energy, 0, opts)
...
my_disp(['### building kd-tree done; #partition ' num2str(length(data.kdtree)) ...
...
%   I_n = 
% end
my_disp(['### building kd-tree done; #partition ' num2str(kdtree_size) ...
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% ADDED BY LUKE TEACY 18/03/2011
function my_disp(varargin)

persistent isDisabled;

if isequal(nargin,2)
   isDisabled=varargin{2};
   return;
end

if ~isequal(true,isDisabled)
   disp(varargin{:});
end
